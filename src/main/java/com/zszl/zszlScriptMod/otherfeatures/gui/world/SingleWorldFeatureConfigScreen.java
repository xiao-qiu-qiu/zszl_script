...
import com.zszl.zszlScriptMod.otherfeatures.handler.world.WorldFeatureManager.FeatureState;
...
public class SingleWorldFeatureConfigScreen extends ThemedGuiScreen {
    ...
    public SingleWorldFeatureConfigScreen(GuiScreen parentScreen, String featureId, String title) {
        this.parentScreen = parentScreen;
        this.featureId = featureId == null ? "" : featureId.trim();
        this.title = title == null ? "世界功能设置" : title.trim();
    }
    ...
    private void computeLayout() {
        ...
        int infoBodyHeight = Math.max(36,
                this.descriptionLines.size() * textStep + 6 + this.fontRenderer.FONT_HEIGHT + 6
    ...
    private String buildInstructionText(FeatureState state) {
        if (state == null) {
            return "这里可以调整当前世界功能的草稿状态与参数，修改后通过底部按钮保存并返回。";
        }
    ...
    private FeatureState getFeature() {
        return WorldFeatureManager.getFeature(this.featureId);
    }
    ...
    private void saveDraft(FeatureState state) {
        ...
        WorldFeatureManager.setEnabled(this.featureId, this.draftEnabled);
        WorldFeatureManager.setFeatureStatusHudEnabled(this.featureId, this.draftStatusHudEnabled);
        if (state.supportsValue()) {
            WorldFeatureManager.setValue(this.featureId, this.draftValue);
        }
    }
    ...
    private void openValueInput(FeatureState state) {
        String titleText = "输入 " + state.valueLabel + " (" + formatFloat(state.minValue) + " - "
                + formatFloat(state.maxValue) + ")";
        mc.displayGuiScreen(new GuiTextInput(this, titleText, formatFloat(this.draftValue), value -> {
    ...
    private void drawStatusStrip(FeatureState state) {
        ...
        String status = "状态: " + (this.draftEnabled ? "§a开启" : "§c关闭")
                ...
                + " §7| 逻辑: " + (state != null && state.behaviorImplemented ? "§a已接入" : "§6占位");
        if (state != null && state.supportsValue()) {
        ...
        try {
            ...
            if (isVisibleInScrollableViewport(this.infoRuntimeY, this.fontRenderer.FONT_HEIGHT + 2)) {
                String runtime = state == null
                        ? "§6运行状态: 未找到对应功能"
                        : "§e运行状态: §f" + WorldFeatureManager.getFeatureRuntimeSummary(this.featureId);

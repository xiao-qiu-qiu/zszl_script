...
import com.zszl.zszlScriptMod.shadowbaritone.process.ElytraProcess;
...
import static com.zszl.zszlScriptMod.shadowbaritone.utils.BaritoneMath.fastCeil;
import static com.zszl.zszlScriptMod.shadowbaritone.utils.BaritoneMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    /**
     * Remaining cool-down ticks after the player's position and rotation are reset
     * by the server
     */
    ...
    /**
     * The most recent minimum number of firework boost ticks, equivalent to
     * {@code 10 * (1 + Flight)}
     * <p>
    ...
    public final class PathManager {
                    ...
                    .thenRun(() -> {
                        ...
                        if (this.completePath) {
                            logDirect(ShadowBaritoneI18n.trKey(
                                    "shadowbaritone.process.elytra.status.computed_path",
                                    distance,
                                    (System.nanoTime() - start) / 1e9d));
                        } else {
                            logDirect(ShadowBaritoneI18n.trKey(
                                    "shadowbaritone.process.elytra.status.computed_segment",
                                    distance,
                                    (System.nanoTime() - start) / 1e9d));
                        }
                    })
                    .whenComplete((result, ex) -> {
                        ...
                        if (ex != null) {
                            ...
                            if (cause instanceof PathCalculationException) {
                                logDirect(ShadowBaritoneI18n.trKey(
                                        "shadowbaritone.process.elytra.error.compute_path_failed"));
                            } else {
        ...
        public CompletableFuture<Void> pathRecalcSegment(final int upToIncl) {
            ...
            return this
                    .path0(ctx.playerFeet(), this.path.get(upToIncl),
                            segment -> segment.append(after.stream(), complete))
                    .whenComplete((result, ex) -> {
                        this.recalculating = false;
                        if (ex != null) {
                            final Throwable cause = ex.getCause();
                            if (cause instanceof PathCalculationException) {
                                logDirect(ShadowBaritoneI18n.trKey(
                                        "shadowbaritone.process.elytra.error.recompute_segment_failed"));
                            } else {
                                logUnhandledException(cause);
                            }
                        }
            ...
            this.path0(pathStart, ElytraBehavior.this.destination, segment -> segment.prepend(before.stream()))
                    ...
                    .whenComplete((result, ex) -> {
                        ...
                        if (ex != null) {
                            ...
                            if (cause instanceof PathCalculationException) {
                                logDirect(ShadowBaritoneI18n.trKey(
                                        "shadowbaritone.process.elytra.error.compute_next_segment_failed"));
                                if (ctx.player().getDistanceSq(pathStart) < 16 * 16) {
                                    logDirect(ShadowBaritoneI18n.trKey(
                                            "shadowbaritone.process.elytra.status.player_near_segment_start"));
                                    completePath = true;
                                }
                            } else {
                                logUnhandledException(cause);
        ...
        private void setPath(final UnpackedSegment segment) {
            ...
            if (ElytraBehavior.this.appendDestination) {
                ...
                if (last != null && ElytraBehavior.this.clearView(new Vec3d(dest), new Vec3d(last), false)) {
                    ...
                    logDirect(ShadowBaritoneI18n.trKey(
                            "shadowbaritone.process.elytra.error.unable_land_at",
        ...
        private void pathfindAroundObstacles() {
            ...
            // rangeEndExcl now represents an index either not in the path, or just outside
            // render distance
            ...
            if (ElytraBehavior.this.process.state != ElytraProcess.State.LANDING && this.ticksNearUnchanged > 100) {
                this.pathRecalcSegment(rangeEndExcl - 1)
                        .thenRun(() -> {
                            logDirect(ShadowBaritoneI18n.trKey(
                                    "shadowbaritone.process.elytra.status.recalculating_no_progress"));
                        });
                this.ticksNearUnchanged = 0;
                return;
            }
            ...
            for (int i = rangeStartIncl; i < rangeEndExcl - 1; i++) {
                if (ElytraBehavior.this.clearView(ctx.playerFeetAsVec(), this.path.getVec(i), false)
                        || ElytraBehavior.this.clearView(ctx.playerHead(), this.path.getVec(i), false)) {
                    canSeeAny = true;
                }
                if (!ElytraBehavior.this.clearView(this.path.getVec(i), this.path.getVec(i + 1), false)) {
                    ...
                    this.pathRecalcSegment(rangeEndExcl - 1)
                            .thenRun(() -> {
                                logDirect(ShadowBaritoneI18n.trKey(
                                        "shadowbaritone.process.elytra.status.recalculated_around_blockage",
            ...
            if (!canSeeAny && rangeStartIncl < rangeEndExcl - 2 && process.state != ElytraProcess.State.GET_TO_JUMP) {
                this.pathRecalcSegment(rangeEndExcl - 1)
                        .thenRun(() -> logDirect(ShadowBaritoneI18n.trKey(
                                "shadowbaritone.process.elytra.status.recalculated_no_visible_points")));
            }
        }
    ...
    public void onChunkEvent(ChunkEvent event) {
        if (event.isPostPopulate() && this.context != null) {
            final Chunk chunk = ctx.world().getChunkFromChunkCoords(event.getX(), event.getZ());
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > Baritone.settings().elytraTimeBetweenCacheCullSecs.value) {
            this.context.queueCacheCulling(ctx.player().chunkCoordX, ctx.player().chunkCoordZ,
                    Baritone.settings().elytraCacheCullDistance.value, this.boi);
    ...
    private void onTick0() {
        ...
        if (this.solver != null) {
            try {
                ...
                // it doesn't matter if get() fails since the solution can just be recalculated
                // synchronously
            } finally {
                this.solver = null;
            }
    ...
    /**
     * Called by
     * {@link com.zszl.zszlScriptMod.shadowbaritone.process.ElytraProcess#onTick(boolean, boolean)}
     * when the process is in control and the player is flying
     */
    public void tick() {
        ...
        if (ctx.player().collidedHorizontally) {
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.elytra.debug.hbonk"));
        }
        if (ctx.player().collidedVertically) {
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.elytra.debug.vbonk"));
        }

        final SolverContext solverContext = this.new SolverContext(false);
        this.solveNextTick = true;

        // If there's no previously calculated solution to use, or the context used at
        // the end of last tick doesn't match this tick
        ...
        if (solution == null) {
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.elytra.error.no_solution"));
            return;
        }
        ...
        if (!solution.solvedPitch) {
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.elytra.error.no_pitch_solution"));
            return;
        } else {
    ...
    public void onPostTick(TickEvent event) {
        if (event.getType() == TickEvent.Type.IN && this.solveNextTick) {
            // We're at the end of the tick, the player's position likely updated and the
            // closest path node could've
            // changed. Updating it now will avoid unnecessary recalculation on the main
            // thread.
    ...
    private Solution solveAngles(final SolverContext context) {
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and
                                                                 // more (if we're in a corner or near some blocks, it
                                                                 // will have to relax its constraints a bit)
            int[] heights = context.boost.isBoosted() ? new int[] { 20, 10, 5, 0 } : new int[] { 0 }; // attempt to gain
                                                                                                      // height, if we
                                                                                                      // can, so as not
                                                                                                      // to waste the
                                                                                                      // boost
            int lookahead = relaxation == 0 ? 2 : 3; // ideally this would be expressed as a distance in blocks, rather
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (int dy : heights) {
                    if (relaxation == 0 || i == minStep) {
                        ...
                        final int steps = fastFloor(delta.lengthVector());
                ...
                for (final Pair<Vec3d, Integer> candidate : candidates) {
                    ...
                    Vec3d dest = candidate.first().addVector(0, augment, 0);
                    if (landingMode) {
                        dest = dest.addVector(0.5, 0.5, 0.5);
                    }
                    ...
                    if (augment != 0) {
                        ...
                        if (start.distanceTo(dest) < 40) {
                            if (!this.clearView(dest, path.getVec(i + lookahead).addVector(0, augment, 0), false)
                                    || !this.clearView(dest, path.getVec(i + lookahead), false)) {
                                // aka: don't go upwards if doing so would prevent us from being able to see the
                                // next position **OR** the modified next position
                                continue;
                            }
                        } else {
                            // but if it's far away, allow gaining altitude if we could lose it again by the
                            // time we get there
                    ...
                    if (this.isHitboxClear(context, dest, growth)) {
                        ...
                        final float yaw = RotationUtils.calcRotationFromVec3d(start, dest, ctx.playerRotations())
                                .getYaw();

                        final Pair<Float, Boolean> pitch = this.solvePitch(context, dest, relaxation);
                        if (pitch == null) {
                            solution = new Solution(context, new Rotation(yaw, ctx.playerRotations().getPitch()), null,
                                    false, false);
                            continue;
                        }
    ...
    private void tickUseFireworks(final Vec3d start, final Vec3d goingTo, final boolean isBoosted,
        ...
        final boolean useOnDescend = !Baritone.settings().elytraConserveFireworks.value
                || ctx.player().posY < goingTo.y + 5;
        final double currentSpeed = new Vec3d(
                ...
                ctx.player().motionZ).lengthSquared();
        ...
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
                && useOnDescend
                && (ctx.player().posY < goingTo.y - 5
                        || start.distanceTo(new Vec3d(goingTo.x + 0.5, ctx.player().posY, goingTo.z + 0.5)) > 5) // UGH!!!!!!!
            ...
            if (!baritone.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                ...
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.elytra.error.no_fireworks"));
                return;
            }
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.elytra.status.attempt_use_firework",
                    forceUseFirework
                            ? ShadowBaritoneI18n.trKey(
                                    "shadowbaritone.process.elytra.value.forced_suffix")
                            : ""));
    ...
    private final class SolverContext {
        ...
        /**
         * Creates a new SolverContext using the current state of the path, player, and
         * firework boost at the time of
         * construction.
         *
         * @param async Whether the computation is being done asynchronously at the end
         *              of a game tick.
         */
        public SolverContext(boolean async) {
            ...
            if (async) {
                // async computation is done at the end of a tick, advance by 1 to prepare for
                // the next tick
                aim.advance(1);
            }
    ...
    private static final class FireworkBoost {
        ...
        /**
         * @param fireworkTicksExisted The ticksExisted of the attached firework entity,
         *                             or {@code null} if no entity.
         * @param minimumBoostTicks    The minimum number of boost ticks that the
         *                             attached firework entity, if any, will
         *                             provide.
         */
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3d goal, final int relaxation) {
        ...
        final float goodPitch = RotationUtils.calcRotationFromVec3d(context.start, goal, ctx.playerRotations())
                .getPitch();
        final FloatArrayList pitches = pitchesToSolveFor(goodPitch, desperate);

        final IntTriFunction<PitchResult> solve = (ticks, ticksBoosted, ticksBoostDelay) -> this.solvePitch(context,
                goal, relaxation, pitches.iterator(), ticks, ticksBoosted, ticksBoostDelay);
        ...
        final int ticks = desperate ? 3
                : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks())
                        : Baritone.settings().elytraSimulationTicks.value;
        ...
        // If we used a firework would we be able to get out of the current situation???
        // perhaps
        if (desperate) {
    ...
    private PitchResult solvePitch(final SolverContext context, final Vec3d goal, final int relaxation,
        ...
        while (pitches.hasNext()) {
            ...
            if (landingMode) {
                goodness = -goalDelta.subtract(last).lengthVector();
            }
        ...
        outer: for (final PitchResult result : bestResults) {
            if (relaxation < 2) {
                ...
                if (!clearView(context.start.add(result.steps.get(result.steps.size() - 1)), goal,
                        context.ignoreLava)) {
                    continue;
                }
            }
    ...
    private List<Vec3d> simulate(final SolverContext context, final Vec3d goalDelta, final float pitch, final int ticks,
        ...
        for (int i = 0; i < ticks; i++) {
            ...
            // Collision box while the player is in motion, with additional padding for
            // safety
    ...
    private static Vec3d step(final Vec3d motion, final Vec3d lookDirection, final float pitch) {
        ...
        double thisIsAlwaysOne = lookDirection.lengthVector();
        float pitchBase3 = MathHelper.cos(pitchRadians);
        // System.out.println("always the same lol " + -pitchBase + " " + pitchBase3);
        // System.out.println("always the same lol " + Math.abs(pitchBase3) + " " +
        // pitchBase2);
        ...
        if (pitchBase2 > 0) { // this is always true unless you are looking literally straight up (let's just
                              // say the bot will never do that)
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getItem().getMaxDamage()
                    - slot.getItemDamage()) > Baritone.settings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        ...
        if (chest.getItem() != Items.ELYTRA
                || chest.getItem().getMaxDamage()
                        - chest.getItemDamage() > Baritone.settings().elytraMinimumDurability.value) {
            return;
        }
    ...
    /**
     * Minecraft 1.12's pushOutOfBlocks logic doesn't account for players being able
     * to fit under single block spaces,
     * so whenever the edge of a ceiling is encountered while elytra flying it tries
     * to push the player out.
     */
    private static final class MC_1_12_Collision_Fix {
        ...
        public static boolean bonk(final BlockStateInterface bsi, final double xIn, final double yIn,
                final double zIn) {
        ...
        private static boolean pushOutOfBlocks(final BlockStateInterface bsi, final int x, final int y, final int z) {
            ...
            if (result == null) {
                PUSH_OUT_CACHE.put(hash, result = !isOpenBlockSpace(bsi, x, y, z) && (isOpenBlockSpace(bsi, x - 1, y, z)
        ...
        private static boolean isOpenBlockSpace(final BlockStateInterface bsi, final int x, final int y, final int z) {
            ...
            if (result == null) {
                IS_OPEN_CACHE.put(hash,
                        result = !bsi.get0(x, y, z).isNormalCube() && !bsi.get0(x, y + 1, z).isNormalCube());
            }
            return result;
        }
    }

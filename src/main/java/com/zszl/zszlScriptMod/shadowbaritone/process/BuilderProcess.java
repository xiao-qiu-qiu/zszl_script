...
import com.zszl.zszlScriptMod.shadowbaritone.api.pathing.goals.GoalGetToBlock;
import com.zszl.zszlScriptMod.shadowbaritone.api.process.IBuilderProcess;
...
import com.zszl.zszlScriptMod.shadowbaritone.api.schematic.SubstituteSchematic;
...
import com.zszl.zszlScriptMod.shadowbaritone.utils.schematic.MapArtSchematic;
import com.zszl.zszlScriptMod.shadowbaritone.utils.schematic.SchematicSystem;
import com.zszl.zszlScriptMod.shadowbaritone.utils.schematic.SelectionSchematic;
import com.zszl.zszlScriptMod.shadowbaritone.utils.schematic.format.defaults.LitematicaSchematic;
import com.zszl.zszlScriptMod.shadowbaritone.utils.schematic.litematica.LitematicaHelper;
import com.zszl.zszlScriptMod.shadowbaritone.utils.schematic.schematica.SchematicaHelper;
...
import static com.zszl.zszlScriptMod.shadowbaritone.api.pathing.movement.ActionCosts.COST_INF;
...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    public void build(String name, ISchematic schematic, Vec3i origin) {
        ...
        if (Baritone.settings().buildOnlySelection.value && buildingSelectionSchematic) { // currently redundant but
                                                                                          // safer maybe
            if (baritone.getSelectionManager().getSelections().length == 0) {
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.error.no_selection_build_only"));
                this.stopAtHeight = 0;
            } else if (Baritone.settings().buildInLayers.value) {
                OptionalInt minim = Stream.of(baritone.getSelectionManager().getSelections())
                        .mapToInt(sel -> sel.min().y).min();
                OptionalInt maxim = Stream.of(baritone.getSelectionManager().getSelections())
                        .mapToInt(sel -> sel.max().y).max();
                if (minim.isPresent() && maxim.isPresent()) {
                    int startAtHeight = Baritone.settings().layerOrder.value
                            ? y + schematic.heightY() - maxim.getAsInt()
                            : minim.getAsInt() - y;
                    this.stopAtHeight = (Baritone.settings().layerOrder.value
                            ? y + schematic.heightY() - minim.getAsInt()
                            : maxim.getAsInt() - y) + 1;
                    this.layer = Math.max(this.layer, startAtHeight / Baritone.settings().layerHeight.value); // startAtLayer
                                                                                                              // or
                                                                                                              // startAtHeight,
                                                                                                              // whichever
                                                                                                              // is
    ...
    public void buildOpenSchematic() {
        if (SchematicaHelper.isSchematicaPresent()) {
            ...
            if (schematic.isPresent()) {
                ...
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.error.no_schematic_open"));
            }
        } else {
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.builder.error.schematica_not_present"));
        }
    }
    ...
    public void buildOpenLitematic(int i) {
        if (LitematicaHelper.isLitematicaPresent()) {
            ...
            if (LitematicaHelper.hasLoadedSchematic()) {
                ...
                try {
                    LitematicaSchematic schematic1 = new LitematicaSchematic(CompressedStreamTools.readCompressed(
                            Files.newInputStream(LitematicaHelper.getSchematicFile(i).toPath())), false);
                    ...
                    logDirect(ShadowBaritoneI18n.trKey(
                            "shadowbaritone.process.builder.error.schematic_file_load_failed"));
                }
            } else {
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.error.no_schematic_loaded"));
            }
        } else {
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.builder.error.litematica_not_present"));
        }
    }
        ...
        for (int dx = -5; dx <= 5; dx++) {
            for (int dy = Baritone.settings().breakFromAbove.value ? -1 : 0; dy <= 5; dy++) {
                for (int dz = -5; dz <= 5; dz++) {
                    ...
                    if (curr.getBlock() != Blocks.AIR && !(curr.getBlock() instanceof BlockLiquid)
    ...
    private Optional<Placement> searchForPlaceables(BuilderCalculationContext bcc,
            List<IBlockState> desirableOnHotbar) {
        BetterBlockPos center = ctx.playerFeet();
        for (int dx = -5; dx <= 5; dx++) {
    ...
    private Optional<Placement> possibleToPlace(IBlockState toPlace, int x, int y, int z, BlockStateInterface bsi) {
        for (EnumFacing against : EnumFacing.values()) {
            ...
            for (Vec3d placementMultiplier : aabbSideMultipliers(against)) {
                ...
                RayTraceResult result = RayTraceUtils.rayTraceTowards(ctx.player(), actualRot,
                        ctx.playerController().getBlockReachDistance(), true);
                if (result != null && result.typeOfHit == RayTraceResult.Type.BLOCK
                        && result.getBlockPos().equals(placeAgainstPos) && result.sideHit == against.getOpposite()) {
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel, int recursions) {
        ...
        if (Baritone.settings().buildInLayers.value) {
            ...
            schematic = new ISchematic() {
                ...
                public IBlockState desiredState(int x, int y, int z, IBlockState current,
                        List<IBlockState> approxPlaceable) {
        ...
        if (!recalc(bcc)) {
            if (Baritone.settings().buildInLayers.value
                    && layer * Baritone.settings().layerHeight.value < stopAtHeight) {
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.status.starting_layer",
                        layer));
                layer++;
            ...
            if (repeat.equals(new Vec3i(0, 0, 0)) || (max != -1 && numRepeats >= max)) {
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.status.done_building"));
                if (Baritone.settings().notificationOnBuildFinished.value) {
                    logNotification(ShadowBaritoneI18n.trKey(
                            "shadowbaritone.process.builder.status.done_building"), false);
                }
                onLostControl();
                return null;
            }
            ...
            logDirect(ShadowBaritoneI18n.trKey(
                    "shadowbaritone.process.builder.status.repeating_build",
                    repeat,
                    origin));
            return onTick(calcFailed, isSafeToCancel, recursions + 1);
        }
        ...
        if (toPlace.isPresent() && isSafeToCancel && ctx.player().onGround && ticks <= 0) {
            ...
            if ((ctx.isLookingAt(toPlace.get().placeAgainst)
                    && ctx.objectMouseOver().sideHit.equals(toPlace.get().side))
        ...
        if (goal == null) {
            ...
            if (goal == null) {
                if (Baritone.settings().skipFailedLayers.value && Baritone.settings().buildInLayers.value
                        && layer * Baritone.settings().layerHeight.value < realSchematic.heightY()) {
                    logDirect(ShadowBaritoneI18n.trKey(
                            "shadowbaritone.process.builder.status.skipping_unconstructable_layer",
                            layer));
                    layer++;
                    return onTick(calcFailed, isSafeToCancel, recursions + 1);
                }
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.error.unable_pausing"));
                paused = true;
                return new PathingCommand(null, PathingCommandType.REQUEST_PAUSE);
            }
        }
            ...
            for (int z = 0; z < schematic.lengthZ(); z++) {
                for (int x = 0; x < schematic.widthX(); x++) {
                    ...
                    if (!observedCompleted.contains(BetterBlockPos.longHash(blockX, blockY, blockZ))
                            && !Baritone.settings().buildSkipBlocks.value.contains(
                                    schematic.desiredState(x, y, z, current, this.approxPlaceable).getBlock())) {
    ...
    private Goal assemble(BuilderCalculationContext bcc, List<IBlockState> approxPlaceable, boolean logMissing) {
        ...
        if (toBreak.isEmpty()) {
            if (logMissing && !missing.isEmpty()) {
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.status.missing_materials"));
            ...
            if (logMissing && !flowingLiquids.isEmpty()) {
                logDirect(ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.status.unreplaceable_liquids"));
    ...
    private Goal placementGoal(BlockPos pos, BuilderCalculationContext bcc) {
        ...
        for (EnumFacing facing : Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP) {
            ...
            if (MovementHelper.canPlaceAgainst(ctx, pos.offset(facing))
                    && ctx.world().mayPlace(bcc.getSchematic(pos.getX(), pos.getY(), pos.getZ(), current).getBlock(),
                            pos, false, facing, null)) {
    ...
    private Goal breakGoal(BlockPos pos, BuilderCalculationContext bcc) {
        if (Baritone.settings().goalBreakFromAbove.value && bcc.bsi.get0(pos.up()).getBlock() instanceof BlockAir
                && bcc.bsi.get0(pos.up(2)).getBlock() instanceof BlockAir) { // TODO maybe possible without the up(2)
                                                                             // check?
    ...
    public String displayName0() {
        return paused
                ? ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.display.paused")
                : ShadowBaritoneI18n.trKey(
                        "shadowbaritone.process.builder.display.building",
                        name);
    }
    ...
    private List<IBlockState> approxPlaceable(int size) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // <toxic cloud>
            result.add(((ItemBlock) stack.getItem()).getBlock().getStateForPlacement(ctx.world(), ctx.playerFeet(),
                    EnumFacing.UP, (float) ctx.player().posX, (float) ctx.player().posY, (float) ctx.player().posZ,
                    stack.getItem().getMetadata(stack.getMetadata()), ctx.player()));
            // </toxic cloud>
        }
        return result;
    }

    public static final Set<IProperty<?>> orientationProps = ImmutableSet.of(BlockRotatedPillar.AXIS, BlockLog.LOG_AXIS,
    ...
    private boolean sameBlockstate(IBlockState first, IBlockState second) {
        ...
        Map map1 = first.getProperties();
        Map map2 = second.getProperties();
        for (Object propObj : map1.keySet()) {
            IProperty<?> prop = (IProperty<?>) propObj;
    ...
    private boolean valid(IBlockState current, IBlockState desired, boolean itemVerify) {
        ...
        if (desired.getBlock() instanceof BlockAir
                && Baritone.settings().buildIgnoreBlocks.value.contains(current.getBlock())) {
            return true;
        }
        ...
        if (Baritone.settings().buildValidSubstitutes.value.getOrDefault(desired.getBlock(), Collections.emptyList())

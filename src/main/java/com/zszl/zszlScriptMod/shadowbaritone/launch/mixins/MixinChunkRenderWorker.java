...
public abstract class MixinChunkRenderWorker {
    ...
    @Shadow(remap = false)
    protected abstract boolean func_188263_a(BlockPos pos, World worldIn);
    ...
    private boolean zszl$isChunkExistingRedirect(ChunkRenderWorker worker, BlockPos pos, World world) {
        if (Baritone.settings().renderCachedChunks.value && !Minecraft.getMinecraft().isSingleplayer()) {
            Baritone baritone = (Baritone) BaritoneAPI.getProvider().getPrimaryBaritone();
            IPlayerContext ctx = baritone.getPlayerContext();
            if (ctx.player() != null && ctx.world() != null && baritone.bsi != null) {
                return baritone.bsi.isLoaded(pos.getX(), pos.getZ()) || this.func_188263_a(pos, world);
            }
        }

        return this.func_188263_a(pos, world);
    }
}

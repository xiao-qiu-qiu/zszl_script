...
import com.zszl.zszlScriptMod.shadowbaritone.cache.CachedRegion;
...
public class BlockStateInterface {
    ...
    private static Field loadedChunksFieldFallback;
    ...
    public BlockStateInterface(IPlayerContext ctx, boolean copyLoadedChunks) {
        ...
        Long2ObjectMap<Chunk> worldLoaded = getLoadedChunksSafe(world);
        if (copyLoadedChunks) {
            this.loadedChunks = new Long2ObjectOpenHashMap<>(worldLoaded); // make a copy that we can safely access from
                                                                           // another thread
        } else {
    ...
    private static Long2ObjectMap<Chunk> getLoadedChunksSafe(World world) {
        if (world == null || world.getChunkProvider() == null) {
            return new Long2ObjectOpenHashMap<>();
        }
        if (world.getChunkProvider() instanceof IChunkProviderClient) {
            return ((IChunkProviderClient) world.getChunkProvider()).loadedChunks();
        }
        try {
            if (loadedChunksFieldFallback == null) {
                for (Field field : world.getChunkProvider().getClass().getDeclaredFields()) {
                    if (Long2ObjectMap.class.isAssignableFrom(field.getType())) {
                        field.setAccessible(true);
                        loadedChunksFieldFallback = field;
                        break;
                    }
                }
            }
            if (loadedChunksFieldFallback != null) {
                Object value = loadedChunksFieldFallback.get(world.getChunkProvider());
                if (value instanceof Long2ObjectMap) {
                    return (Long2ObjectMap<Chunk>) value;
                }
            }
    ...
    public static Block getBlock(IPlayerContext ctx, BlockPos pos) { // won't be called from the pathing thread because
                                                                     // the pathing thread doesn't make a single
                                                                     // blockpos pog
    ...
    public static IBlockState get(IPlayerContext ctx, BlockPos pos) {
        ...
        // and toBreak and stuff fails when the movement is instantiated out of load
        // range but it's not able to BlockStateInterface.get what it's going to walk on
    }

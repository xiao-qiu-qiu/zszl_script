...
public class GuiClick extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (near != null && far != null) {
            Vec3d viewerPos = new Vec3d(mc.getRenderManager().viewerPosX, mc.getRenderManager().viewerPosY,
                    mc.getRenderManager().viewerPosZ);
            RayTraceResult result = mc.world.rayTraceBlocks(near.add(viewerPos), far.add(viewerPos), false, false,
                    true);
            ...
            if (mouseButton == 0) {
                if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                    ...
                    ITextComponent component = new TextComponentString(
                            "Selection made! For usage: " + Baritone.settings().prefix.value + "help sel");
                    component.getStyle()
                    ...
                    BaritoneAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess()
                            .setGoalAndPath(GoalTargetNormalizer.normalizeGoalBlock(
                                    BaritoneAPI.getProvider().getPrimaryBaritone(),
                                    new GoalBlock(currentMouseOver)));
                }
            } else if (mouseButton == 1) {
                BaritoneAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess()
                        .setGoalAndPath(GoalTargetNormalizer.normalizeGoalBlock(
                                BaritoneAPI.getProvider().getPrimaryBaritone(),
                                new GoalBlock(currentMouseOver.up())));
            }
        }
        clickStart = null;
    }
    ...
    public void onRender() {
        ...
        if (currentMouseOver != null) {
            ...
            if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                ...
                IRenderer.emitAABB(new AxisAlignedBB(Math.min(a.x, b.x), Math.min(a.y, b.y), Math.min(a.z, b.z),
    ...
    private Vec3d toWorld(double x, double y, double z) {
        boolean result = GLU.gluUnProject((float) x, (float) y, (float) z, MODELVIEW, PROJECTION, VIEWPORT,

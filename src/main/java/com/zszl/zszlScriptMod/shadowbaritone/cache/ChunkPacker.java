...
import com.zszl.zszlScriptMod.shadowbaritone.utils.pathing.PathingBlockType;
...
public final class ChunkPacker {
    ...
    private static PathingBlockType getPathingBlockType(IBlockState state, Chunk chunk, int x, int y, int z) {
        ...
        if (block == Blocks.WATER || block == Blocks.FLOWING_WATER) {
            ...
            // FLOWING_WATER is a waterfall, it doesn't really matter and caching it as
            // AVOID just makes it look wrong
            if (MovementHelper.possiblyFlowing(state)) {
                return PathingBlockType.AVOID;
            }
            if ((x != 15 && MovementHelper.possiblyFlowing(chunk.getBlockState(x + 1, y, z)))
            ...
            if (x == 0 || x == 15 || z == 0 || z == 15) {
                if (BlockLiquid.getSlopeAngle(chunk.getWorld(), new BlockPos(x + (chunk.x << 4), y, z + (chunk.z << 4)),
                        state.getMaterial(), state) == -1000.0F) {
        ...
        // because fences check their adjacent blocks in the world for their fence
        ...
        if (block == Blocks.AIR || block instanceof BlockTallGrass || block instanceof BlockDoublePlant

...
import com.zszl.zszlScriptMod.shadowbaritone.api.cache.ICachedRegion;
...
public final class CachedRegion implements ICachedRegion {
    ...
    /**
     * Magic value to detect invalid cache files, or incompatible cache files saved
     * in an old version of Baritone
     */
                ...
                for (int x = 0; x < 32; x++) {
                    for (int z = 0; z < 32; z++) {
                        if (present[x][z]) {
                            ...
                            // ^ haha jokes on leijurv, java doesn't have unsigned types so that isn't
                            // correct
                ...
                for (int x = 0; x < 32; x++) {
                    for (int z = 0; z < 32; z++) {
                        if (present[x][z]) {
                            ...
                            this.chunks[x][z] = new CachedChunk(chunkX, chunkZ, bitSets[x][z], overview[x][z],
                                    location[x][z], cacheTimestamp[x][z]);
                        }
                    }
                }
            }
        ...
        for (int x = 0; x < 32; x++) {
            for (int z = 0; z < 32; z++) {
                if (this.chunks[x][z] != null && this.chunks[x][z].cacheTimestamp < oldestAcceptableAge) {
                    System.out.println("Removing chunk " + (x + 32 * this.x) + "," + (z + 32 * this.z)
                            + " because it was cached " + (now - this.chunks[x][z].cacheTimestamp) / 1000L

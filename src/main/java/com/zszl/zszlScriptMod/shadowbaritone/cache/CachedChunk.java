...
public final class CachedChunk {
    ...
    public final IBlockState getBlock(int x, int y, int z, int dimension) {
        ...
        if (heightMap[internalPos] == y && type != PathingBlockType.AVOID) {
            // if the top block in a column is water, we cache it as AVOID but we don't want
            // to just return default state water (which is not flowing) beacuse then it
            // would try to path through it

            // we have this exact block, it's a surface block
            /*
             * System.out.println("Saying that " + x + "," + y + "," + z + " is " + state);
             * if (!Minecraft.getMinecraft().world.getBlockState(new BlockPos(x + this.x *
             * 16, y, z + this.z * 16)).getBlock().equals(state.getBlock())) {
             * throw new IllegalStateException("failed " +
             * Minecraft.getMinecraft().world.getBlockState(new BlockPos(x + this.x * 16, y,
             * z + this.z * 16)).getBlock() + " " + state.getBlock() + " " + (x + this.x *
             * 16) + " " + y + " " + (z + this.z * 16));
             * }
             */

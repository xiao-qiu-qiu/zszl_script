...
public class MovementFall extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (playerFeet.equals(dest) && (ctx.player().posY - playerFeet.getY() < 0.094 || isWater)) { // 0.094 because
                                                                                                     // lilypads
        ...
        if (Math.abs(ctx.player().posX + ctx.player().motionX - destCenter.x) > 0.1
                || Math.abs(ctx.player().posZ + ctx.player().motionZ - destCenter.z) > 0.1) {
        ...
        if (avoid == null) {
            ...
            double dist = Math.abs(avoid.getX() * (destCenter.x - avoid.getX() / 2.0 - ctx.player().posX))
                    + Math.abs(avoid.getZ() * (destCenter.z - avoid.getZ() / 2.0 - ctx.player().posZ));
            if (dist < 0.6) {
                state.setInput(Input.MOVE_FORWARD, true);
    ...
    public boolean safeToCancel(MovementState state) {
        // if we haven't started walking off the edge yet, or if we're in the process of
        // breaking blocks before doing the fall

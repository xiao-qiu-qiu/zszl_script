...
public class MovementPillar extends Movement {
    ...
    public static double cost(CalculationContext context, int x, int y, int z) {
        ...
        if (!ladder) {
            ...
            if (fromDown.getBlock() instanceof BlockSlab && !((BlockSlab) fromDown.getBlock()).isDouble()
                    && fromDown.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.BOTTOM) {
                return COST_INF; // can't pillar up from a bottom slab onto a non ladder
            }
        }
        if (from == Blocks.VINE && !hasAgainst(context, x, y, z)) { // TODO this vine can't be climbed, but we could
                                                                    // place a pillar still since vines are replacable,
                                                                    // no? perhaps the pillar jump would be impossible
                                                                    // because of the slowdown actually.
            return COST_INF;
        }
        ...
        Block srcUp = null;
        if (MovementHelper.isWater(toBreakBlock) && MovementHelper.isWater(from)) { // TODO should this also be allowed
                                                                                    // if toBreakBlock is air?
        ...
        if (from instanceof BlockLiquid || (fromDown.getBlock() instanceof BlockLiquid && context.assumeWalkOnWater)) {
            ...
            // if we're standing on water and assumeWalkOnWater is false, we must have
            // ascended to here, or sneak backplaced, so it is possible to pillar again
            return COST_INF;
        }
        ...
        if (hardness != 0) {
            if (toBreakBlock == Blocks.LADDER || toBreakBlock == Blocks.VINE) {
                hardness = 0; // we won't actually need to break the ladder / vine because we're going to use
                              // it
            } else {
                IBlockState check = context.get(x, y + 3, z); // the block on top of the one we're going to break, could
                                                              // it fall on us?
                ...
                // this is commented because it may have had a purpose, but it's very unclear
                // what it was. it's from the minebot era.
                ...
                // BlockFalling makes sense, and !canWalkOn deals with weird cases like if it
                // were lava
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (MovementHelper.isWater(fromDown.getBlock()) && MovementHelper.isWater(ctx, dest)) {
            ...
            state.setTarget(new MovementState.MovementTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(),
                    VecUtils.getBlockPosCenter(dest), ctx.playerRotations()), false));
        ...
        if (ladder) {
            ...
            double flatMotion = Math
                    .sqrt(ctx.player().motionX * ctx.player().motionX + ctx.player().motionZ * ctx.player().motionZ);
            if (dist > 0.17) {// why 0.17? because it seemed like a good number, that's why
                // [explanation added after baritone port lol] also because it needs to be less
                // than 0.2 because of the 0.3 sneak limit
                // and 0.17 is reasonably less than 0.2

                // If it's been more than forty ticks of trying to jump and we aren't done yet,
                // go forward, maybe we are stuck
                state.setInput(Input.MOVE_FORWARD, true);

...
package com.zszl.zszlScriptMod.shadowbaritone.api.command.argument;

import com.zszl.zszlScriptMod.shadowbaritone.api.command.ICommand;
import com.zszl.zszlScriptMod.shadowbaritone.api.command.argparser.IArgParser;
import com.zszl.zszlScriptMod.shadowbaritone.api.command.datatypes.IDatatype;
import com.zszl.zszlScriptMod.shadowbaritone.api.command.datatypes.IDatatypeFor;
import com.zszl.zszlScriptMod.shadowbaritone.api.command.datatypes.IDatatypePost;
...
import com.zszl.zszlScriptMod.shadowbaritone.api.command.exception.CommandTooManyArgumentsException;
 ...
 * gradually consume arguments in any way
 * you'd like. You can change your consumption based on earlier arguments, for
 * subcommands for example.</li>
 * <li>You don't need to keep track of your consumption. The
 * {@link IArgConsumer}} keeps track of the arguments you
 * consume so that it can throw detailed exceptions whenever something is out of
 * the ordinary. Additionally, if you
 * need to retrieve an argument after you've already consumed it - look no
 * further than {@link #consumed()}!</li>
 * <li>Easy retrieval of many different types. If you need to retrieve an
 * instance of an int or float for example,
 * look no further than {@link #getAs(Class)}. If you need a more powerful way
 * of retrieving data, try out the many
 ...
 * enforce the number of arguments, the type of arguments, and more, throwing
 * different types of
 * {@link CommandException}s if something seems off. You're recommended to do
 * all validation and store all needed
 * data in variables BEFORE logging any data to chat via
 * {@link Helper#logDirect(String)}, so that the error
...
public interface IArgConsumer {
     ...
     * @return {@code true} if there are <i>at least</i> {@code num} arguments left
     *         in this {@link IArgConsumer}}
     * @see #hasAny()
     * @see #hasAtMost(int)
     * @see #hasExactly(int)
     */
    ...
    /**
     * @param num The number of arguments to check for
     * @return {@code true} if there are <i>at most</i> {@code num} arguments left
     *         in this {@link IArgConsumer}}
     * @see #has(int)
     * @see #hasAtMost(int)
     * @see #hasExactly(int)
    ...
    /**
     * @param num The number of arguments to check for
     * @return {@code true} if there are <i>exactly</i> {@code num} arguments left
     *         in this {@link IArgConsumer}}
     * @see #has(int)
     * @see #hasAtMost(int)
     */
     ...
     * @return The argument at index {@code index} in this {@link IArgConsumer}},
     *         with 0 being the next one. This does not
     *         mutate the {@link IArgConsumer}}
     * @throws CommandNotEnoughArgumentsException If there is less than
     *                                            {@code index + 1} arguments left
     * @see #peek()
     * @see #peekString(int)
    ...
    /**
     * @return The next argument in this {@link IArgConsumer}}. This does not mutate
     *         the {@link IArgConsumer}}
     * @throws CommandNotEnoughArgumentsException If there is less than one argument
     *                                            left
     * @see #peek(int)
     * @see #peekString()
     * @see #peekAs(Class)
    ...
    /**
     * @param index The index to peek
     * @return The value of the argument at index {@code index} in this
     *         {@link IArgConsumer}}, with 0 being the next one
    ...
    /**
     * @return The value of the next argument in this {@link IArgConsumer}}. This
     *         does not mutate the {@link IArgConsumer}}
     * @throws CommandNotEnoughArgumentsException If there is less than one argument
     *                                            left
     * @see #peekString(int)
     * @see #getString()
     ...
     * @return From the specified enum class, an enum constant of that class. The
     *         enum constant's name will match the
    ...
    /**
     * Tries to use a <b>stateless</b> {@link IArgParser} to parse the argument at
     * the specified index into the specified
     * class
     * <p>
     * A critical difference between {@link IDatatype}s and {@link IArgParser}s is
     * how many arguments they can take.
     * While {@link IArgParser}s always operate on a single argument's value,
     * {@link IDatatype}s get access to the entire
     * {@link IArgConsumer}}.
     *
    ...
    /**
     * Tries to use a <b>stateless</b> {@link IArgParser} to parse the next argument
     ...
     * @param index The index to peek
     * @return An instance of the specified type, or {@code def} if it couldn't be
     *         parsed
     * @see IArgParser
     * @see #peekAsOrDefault(Class, Object)
     * @see #peekAs(Class, int)
     ...
     * @param type  The type to peek as
     * @param index The index to peek
     * @return An instance of the specified type, or {@code null} if it couldn't be
     *         parsed
     * @see IArgParser
     * @see #peekAsOrNull(Class)
     * @see #peekAs(Class, int)
     ...
     * {@link IArgConsumer}}.
     * <p>
     * Since this is a peek operation, this ArgConsumer will not be mutated by any
     * call to this method.
     *
    ...
    /**
     * Gets the next argument and returns it. This consumes the first argument so
     * that subsequent calls will return
     * later arguments
     *
     * @return The next argument
     * @throws CommandNotEnoughArgumentsException If there's less than one argument
     *                                            left
     */
    ICommandArgument get() throws CommandNotEnoughArgumentsException;

    /**
     * Gets the value of the next argument and returns it. This consumes the first
     * argument so that subsequent calls
     ...
     * argument's value
     * <p>
     * For example if you getEnum as an {@link EnumFacing}, and the next argument's
     * value is "up", this will return
     * {@link EnumFacing#UP}
     *
     ...
     * @param def       The default value
     * @return An enum constant of that class with the same name as the next
     *         argument's value, or {@code def} if it
     *         couldn't be found
     * @see #getEnum(Class)
     * @see #getEnumOrNull(Class)
     * @see #peekEnumOrNull(Class)
     ...
     * @param enumClass The enum class to search
     * @return An enum constant of that class with the same name as the next
     *         argument's value, or {@code null} if it
     *         couldn't be found
     * @see #getEnum(Class)
     ...
     * <code>arg1 arg2&nbsp;&nbsp;arg3</code>, split
     * into three {@link ICommandArgument}s {@code "arg1"}, {@code "arg2"}, and
     * {@code "arg3"}:
     *
     * <ul>
     * <li>{@code rawRest()} would return
     * <code>arg1 arg2&nbsp;&nbsp;arg3</code></li>
     * <li>After calling {@link #get()}, {@code rawRest()} would return
     * <code>arg2&nbsp;&nbsp;arg3</code> (note the
     * double space - it is preserved!)</li>
     * <li>After calling {@link #get()} again, {@code rawRest()} would return
     * {@code "arg3"}</li>
     * <li>After calling {@link #get()} one last time, {@code rawRest()} would
     * return {@code ""}</li>
     * </ul>
     *
     ...
     * @throws CommandNotEnoughArgumentsException If there are less than {@code min}
     *                                            arguments left.
     * @see #requireMax(int)
     ...
     * @throws CommandTooManyArgumentsException If there are more than {@code max}
     *                                          arguments left.
     * @see #requireMin(int)
     * @see #requireExactly(int)
     */
     ...
     * @throws CommandNotEnoughArgumentsException If there are less than
     *                                            {@code args} arguments left.
     * @throws CommandTooManyArgumentsException   If there are more than
     *                                            {@code args} arguments left.
     * @see #requireMin(int)
     * @see #requireMax(int)
     */
     ...
     *         "unknown" argument, indicated by a
     *         comamnd argument index that has a value of {@code -1}, if no
     *         arguments have been consumed yet.
     * @see #consumedString()
     * @see #hasConsumed()
     */
     ...
     *         consumed, or an empty string if no arguments
    ...
    /**
     * @return A copy of this {@link IArgConsumer}}. It has the same arguments (both
     *         consumed and not), but does not
     *         affect or mutate this instance. Useful for the various {@code peek}
     *         functions
     */
    IArgConsumer copy();
}

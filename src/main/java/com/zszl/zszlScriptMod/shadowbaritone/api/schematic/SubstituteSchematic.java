...
public class SubstituteSchematic extends AbstractSchematic {
    ...
    public IBlockState desiredState(int x, int y, int z, IBlockState current, List<IBlockState> approxPlaceable) {
        ...
        if (substitutes.contains(current.getBlock()) && !(current.getBlock() instanceof BlockAir)) {// don't preserve
                                                                                                    // air, it's almost
                                                                                                    // always there and
                                                                                                    // almost never
                                                                                                    // wanted
        ...
        for (Block substitute : substitutes) {
            if (substitute instanceof BlockAir) {
                return current.getBlock() instanceof BlockAir ? current : Blocks.AIR.getDefaultState(); // can always
                                                                                                        // "place" air
            }
    ...
    private <T extends Comparable<T>> IBlockState copySingleProp(IBlockState fromState, IBlockState toState,
            IProperty<T> prop) {

...
package com.zszl.zszlScriptMod.shadowbaritone.api.command.helpers;
...
public class Paginator<E> implements Helper {
            ...
            prevPageComponent.getStyle()
                    ...
                    .setHoverEvent(new HoverEvent(
                            ...
                            new TextComponentString(ShadowBaritoneI18n.trKey(
                                    "shadowbaritone.pagination.hover.previous_page"))));
        } else {
        ...
        if (hasNextPage) {
            nextPageComponent.getStyle()
                    ...
                    .setHoverEvent(new HoverEvent(
                            HoverEvent.Action.SHOW_TEXT,
                            new TextComponentString(ShadowBaritoneI18n.trKey(
                                    "shadowbaritone.pagination.hover.next_page"))));
        } else {
    ...
    public static <T> void paginate(IArgConsumer consumer, Paginator<T> pagi, Runnable pre,
            Function<T, ITextComponent> transform, String commandPrefix) throws CommandException {
        int page = 1;
    ...
    public static <T> void paginate(IArgConsumer consumer, List<T> elems, Runnable pre,
            Function<T, ITextComponent> transform, String commandPrefix) throws CommandException {
        paginate(consumer, new Paginator<>(elems), pre, transform, commandPrefix);
    }

    public static <T> void paginate(IArgConsumer consumer, T[] elems, Runnable pre,
            Function<T, ITextComponent> transform, String commandPrefix) throws CommandException {
        paginate(consumer, Arrays.asList(elems), pre, transform, commandPrefix);
    }

    public static <T> void paginate(IArgConsumer consumer, Paginator<T> pagi, Function<T, ITextComponent> transform,
            String commandPrefix) throws CommandException {
        paginate(consumer, pagi, null, transform, commandPrefix);
    }

    public static <T> void paginate(IArgConsumer consumer, List<T> elems, Function<T, ITextComponent> transform,
            String commandPrefix) throws CommandException {
        paginate(consumer, new Paginator<>(elems), null, transform, commandPrefix);
    }

    public static <T> void paginate(IArgConsumer consumer, T[] elems, Function<T, ITextComponent> transform,
            String commandPrefix) throws CommandException {

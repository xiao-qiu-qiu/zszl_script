 ...
 * state for a given position relative to
 * the origin.
 *
 * @author leijurv
 */
public interface ISchematic {
     ...
     * level of the map art doesn't matter,
     * so this function should return false in that case. (i.e. it doesn't really
     * have to be air below the art blocks)
     *
    ...
    /**
     * Returns the desired block state at a given (X, Y, Z) position relative to the
     * origin (0, 0, 0).
     *
     * @param x               The x position of the block, relative to the origin
     * @param y               The y position of the block, relative to the origin
    ...
    /**
     * Resets possible caches to avoid wrong behavior when moving the schematic
     * around
     */
    default void reset() {
    }
